...
export default function MagazinePost() {
  ...
  useEffect(() => {
    const loadPost = async () => {
      ...
      if (category) {
        try {
          const res = await fetch(`/blog/vi/${category}/${slug}.json`);
          if (res.ok) {
          ...
          console.log('Không tải được từ danh mục đã chọn');
        }
      }

      for (const cat of ['news', 'life', 'hotplace']) {
        try {
          const res = await fetch(`/blog/vi/${cat}/${slug}.json`);
          if (res.ok) {
            const data = await res.json();
  ...
  if (error || !post) {
    return (
      <div className="magazine-post-error">
        ...
        <Link href="/vi/magazine" className="back-link">Quay lại danh sách</Link>
      </div>
    );
  }
      ...
      <article className="post-content-section">
        <div className="post-content-container">
          ...
          <div className="post-navigation">
            <Link href="/vi/magazine" className="back-to-list">← Quay lại danh sách</Link>
          </div>
        </div>
      </article>
    </>
